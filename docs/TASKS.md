...
- **진척**:
  ...
  - Cycle 61 — 지켜보는 페이지 발행 알림 (page.updated). WatchList(Cycle 53)
    ...
    (followup: 지켜보기 버튼은 '개발 중'으로 비활성화 — BE 인프라는 유지)
  - Cycle 62 — 편집기 마무리 3종 (Cycle 54 후속): slash '/이미지' 통합
  ...
  - Cycle 65 — UI 전반 아이콘 일관화 (액션바·편집 툴바·사이드바의 이모지를
    아이콘 자산으로 교체, 공용 아이콘 컴포넌트 도입).
  - Cycle 66 — 빈 본문 페이지를 편집할 때마다 같은 내용이 한 벌씩 늘어나던
    버그 수정 (협업 에디터 초기 로딩 타이밍 문제).
  - Cycle 67 — 편집 툴바 '＋ 더 많은 내용 삽입'에 드롭다운 화살표 표시 +
    툴바에 이미 있는 항목은 그 메뉴에서 빼 중복 정리.
  - Cycle 68 — 인라인 댓글 기능 전체 제거 (거의 안 쓰이고 고장처럼 보여 롤백 —
    일반 댓글·반응은 유지).
  - Cycle 69 — '나중을 위해 저장'을 별표 아이콘으로 바꾸고, 저장한 페이지가
    홈 '나중을 위해 저장' 목록에 실제로 표시되도록 서버 연동.
  - Cycle 70 — 페이지 작업 상태(To Do / In Progress / Done) 배지 도입 — 제목
    옆 표시 + 작성자/관리자 변경 (필터·알림은 후속).
  - Cycle 71 — 페이지 목록 상태 필터 + 스페이스 칸반 보드(드래그로 상태 변경).
  - Cycle 73 — 칸반 보드에 작성자/편집자 사용자 필터 추가.
  - Cycle 76 (일부) — 조회 화면 댓글을 페이지 최하단으로 이동, 사이드바 공간
    헤더의 설명 제거, 편집 모드에서 상단 메뉴가 안 눌리던 문제 수정, 홈
    사이드바 '+ 내 공간 추가' 버튼 제거, 공간 홈 페이지를 페이지 트리에 표시
    (공간 이름 클릭 시 홈 이동). 공간 도구 권한 부트스트랩은 Task I 참조.
  - Cycle 77 — 페이지 하단 '처음으로 좋아하는…' 안내 제거(이모지 반응으로
    대체) + 사이드바 트리에 마우스 올릴 때 뜨던 ＋/× 버튼 제거.
  - Cycle 78 (일부) — 페이지 레이블(태그) 기능 추가 (조회 화면에서 추가/제거).
    공간 디렉터리/페이지 순서/아이콘 기본값은 Task I 참조.
  - Cycle 79 (일부) — 작업 상태에 'Drop' 추가 + 편집 툴바 아이콘 교체(텍스트색/
    배경색/이미지/표) + 제목 메뉴에 '문단' 추가.
  - Cycle 80 (일부) — '만들기' 메뉴를 모달로 전환 (카드형 '빈 페이지', 향후
    템플릿 자리) + 브라우저 기본 입력창(prompt) 제거. 공간 만들기·스페이스
    키는 Task I 참조.
  - Cycle 81 — 페이지 조회 헤더 정리 — 브레드크럼 줄에 '제한' 버튼 + 편집/저장/
    지켜보기/공유/더보기를 한 줄 우측 정렬, 제목은 아래 줄로.
- **남은 일**:
  - Sidebar '즐겨찾기' 섹션이 비어 있음 — 서버 저장 목록과 통합 필요 (Cycle 69)
  - 페이지 상태 변경 알림 / 사이드바 상태별 카운트 / 개인 대시보드 — 별도 사이클
    (Cycle 70·71·73)
  - 페이지 템플릿 (만들기 모달의 카드) (Cycle 80)
  - 백엔드 인라인 댓글 API 잔존 경로 정리 (Cycle 68, 무해)
- **닫힘 이력**: (v1 retro 미수행)
- **차단 / 의존**: 없음
- **Jira Epic**: 미등록
- **관련 Cycle**: Cycle 1~41, 49, 50, 51, 52, 53, 54-D, 54-A, 54-B, 54-F, 54-C, 56, 55, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 73, 76, 77, 78, 79, 80, 81
...
- **상태**: 🟡 Maintenance
- **진척**:
  - 코어 사이클들에서 협업 기반 구축 (Yjs/Hocuspocus 통합, Redis 어댑터
    옵션화 등 — Task A 의 Cycle 39 참조).
  - Cycle 72 — 협업 커서·작성자명에 익명 랜덤 이름('부지런한 다람쥐' 류) 대신
    로그인 실명 표시 (인증 도입 전 잔재 정리).
- **남은 일**:
  - 다이어그램(Excalidraw) 동시 편집 미지원 — 현재 last-write-wins. Task G 와
    연계해 별도 사이클에서 검토.
  - 서버가 클라이언트 전송 작성자명 대신 로그인 사용자(JWT)를 강제로 기록하도록
    굳히기 (Cycle 72)
- **닫힘 이력**: (해당 없음)
- **차단 / 의존**: 없음
- **Jira Epic**: 미등록
- **관련 Cycle**: 코어 사이클들 + Cycle 72 (상세 CYCLES.md)
...
## Task I — 스페이스 권한 · 공간 도구

- **범위**: 스페이스 단위 권한(멤버·역할·공개 범위)과 '공간 도구' 관리 화면
  (개요·권한·감사 로그·페이지 순서·사이드바 구성·아이콘), 스페이스 키.
- **상태**: 🟢 Active
- **진척**:
  - Cycle 74-A — 스페이스 권한 모델 기반: 멤버·역할(관리자/편집자/뷰어)·공개
    범위(전체공개/비공개/개인). 권한 판정 + 읽기/쓰기 가드, 공간 생성자 자동
    관리자. 기존 공간은 전체공개로 백필돼 동작 변화 없음 + 타인 개인 공간이
    목록/검색에 새지 않도록 차단.
  - Cycle 74-B — '공간 도구' 진입점 + 개요 탭 (이름/설명/공개 범위/삭제).
  - Cycle 74-C — 권한 탭: 멤버 추가·역할 변경·제거 + 마지막 관리자 보호.
  - Cycle 74-D — 감사 로그 탭: 스페이스 활동 이력(분류/기간/사용자 필터).
    재발행을 '수정'으로 구분해 생성/수정/삭제가 갈리도록 보강.
  - Cycle 74-E — 페이지 순서 탭: 드래그 순서 변경 + 들여쓰기/내어쓰기, 홈 표시.
  - Cycle 74-F — 사이드바 구성 탭: 공간 바로가기(내부 페이지·외부 URL) 추가·정렬.
  - Cycle 74-G — 스페이스 아이콘(이모지/이미지) 설정 + 목록·사이드바 표시.
  - Cycle 75 — 개요 탭 보기/편집 모드 분리 + 삭제 버튼 분리, 권한 탭 멤버
    다중 선택 제거.
  - Cycle 76 (일부) — 개인 공간도 소유자를 관리자로 등록해 공간 도구·권한 탭을
    쓸 수 있게.
  - Cycle 78 (일부) — 페이지 순서 탭에서 홈도 조작 가능, 개요 아이콘 기본값
    (파란 배경 첫 글자), 공간 목록 비활성 탭 정리.
  - Cycle 79 (일부) — 페이지 순서 들여쓰기를 첫 항목(홈 포함)도 가능하도록 일반화.
  - Cycle 80 (일부) — '공간 만들기'를 모달로 전환(이름·스페이스 키·설명) +
    스페이스 키 도입.
- **남은 일**:
  - 페이지 단위 접근 제한(restrict) — 현재 표시/안내만, 실제 접근 제어 미구현
    (Cycle 81)
  - 페이지 작업 상태 변경 권한이 임시(작성자/관리자) — 정식 권한 정책으로 교체
    (Cycle 70)
  - 일부 쓰기 경로(첨부 업로드/삭제 등) 권한 가드 보강 + 휴지통/다이어그램/버전
    목록 읽기 필터 (Cycle 74-A 후속)
  - 멤버/권한 변경 자체의 감사 로그 미기록 (Cycle 74-D)
  - 사이드바 표시 항목 토글(페이지 트리/최근 활동/즐겨찾기) 미구현 (Cycle 74-F)
- **닫힘 이력**: (해당 없음)
- **차단 / 의존**: 없음
- **Jira Epic**: 미등록
- **관련 Cycle**: Cycle 74-A, 74-B, 74-C, 74-D, 74-E, 74-F, 74-G, 75, 76, 78, 79, 80
