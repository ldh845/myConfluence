...
- **범위**: 기존 3계층 권한(전역 role(48) / 스페이스 멤버십·공개범위(74-A) /
  ...
  그룹(부서/팀) 단위로 확장한다.
- **상태**: 🟢 Active (L5 ✅ — 보안 핵심 폐쇄 완료, 다음은 L5-2 정책 검토)
- **하위 사이클**:
  - **L5** — 권한 가드 구멍 보강: 무인증 쓰기(🔴4) + 데이터 누수 읽기(🟠10) 폐쇄
    + `assertCanViewPage` 프리미티브. ✅ Done
  - **L5-2** — 인증됐으나 공간/페이지 권한 미검사 쓰기(🟡: 댓글 작성/수정/삭제·resolve,
    리액션 toggle, watch, page-share 발급/회전/취소, 페이지 상태 변경, setHomePage)
    보강. 정책 판단(뷰어 댓글 허용 범위·댓글 수정 권한·상태 변경 권한 등) 동반. 📝 Planned
...
- **남은 일**:
  - L5 VM 검증(보강된 구멍 기준) — 아래 시나리오 ①~⑥.
  - L5-2(🟡 쓰기 권한) 정책 검토 후 별도 사이클로 진행.
  - L6~L8(그룹 권한) 설계.
- **L5 VM 검증 시나리오**:
  - ① 비멤버 계정으로 비공개 공간 페이지에 첨부 업로드(`POST /api/pages/:id/attachments`)
    → 403, 디스크/DB 미기록.
  - ② 비멤버로 비공개 공간 첨부 다운로드(`GET /api/attachments/:id`) → 403(파일 미전송).
  - ③ 비멤버로 비공개 페이지 다이어그램 `PATCH /api/diagrams/:id` → 403, 내용 무변화.
  - ④ 비멤버 계정의 휴지통(`GET /api/pages/trash`)·활동 피드(`GET /api/activities`)에
    비공개 공간 항목 미노출.
  - ⑤ 비멤버로 비공개 페이지 버전 이력(`GET /api/pages/:id/versions`)·댓글 목록 → 403.
  - ⑥ 전체공개(PUBLIC) 공간은 위 흐름 전부 기존대로 정상(첨부 업/다운로드·다이어그램·
    버전·댓글 열람) — 회귀 없음 확인.
